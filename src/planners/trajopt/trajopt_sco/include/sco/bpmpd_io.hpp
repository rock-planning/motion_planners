...
#pragma GCC diagnostic ignored "-Wunused-variable"
  ...
  struct bpmpd_input
  {
    ...
    bpmpd_input(int m, int n, int nz, int qn, int qnz,
                ...
                const vector<double> &rhs, const vector<double> &obj, const vector<double> &lbound, const vector<double> &ubound) : m(m), n(n), nz(nz), qn(qn), qnz(qnz), acolcnt(acolcnt), acolidx(acolidx), acolnzs(acolnzs), qcolcnt(qcolcnt), qcolidx(qcolidx), qcolnzs(qcolnzs),
  ...
  struct bpmpd_output
  {
    ...
    double opt;
    bpmpd_output() {}
    bpmpd_output(const vector<double> &primal, const vector<double> &dual, const vector<int> &status, int code, double opt) : primal(primal), dual(dual), status(status), code(code), opt(opt) {}
  };
