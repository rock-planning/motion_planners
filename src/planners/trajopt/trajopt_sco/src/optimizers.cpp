...
BasicTrustRegionSQP::BasicTrustRegionSQP(OptProbPtr prob, SQPConfig config) {
    sqp_config = config;
  setProblem(prob);
}
...
void BasicTrustRegionSQP::adjustTrustRegion(double ratio) {
  sqp_config.trust_box_size_ *= ratio;
}
void BasicTrustRegionSQP::setTrustBoxConstraints(const DblVec& x) {
  ...
  for (size_t i=0; i < x.size(); ++i) {
    lbtrust[i] = fmax(x[i] - sqp_config.trust_box_size_, lb[i]);
    ubtrust[i] = fmin(x[i] + sqp_config.trust_box_size_, ub[i]);
  }
...
OptStatus BasicTrustRegionSQP::optimize() {
  ...
  for (int merit_increases=0; merit_increases < sqp_config.max_merit_coeff_increases_; ++merit_increases) { /* merit adjustment loop */
    for (int iter=1; ; ++iter) { /* sqp loop */
      ...
      vector<ConvexObjectivePtr> cnt_cost_models = cntsToCosts(cnt_models, sqp_config.merit_error_coeff_, model_.get());
      model_->update();
...
//    }
      ...
      while (sqp_config.trust_box_size_ >= sqp_config.min_trust_box_size_) {
        ...
        if (GetLogLevel() >= util::LevelDebug) {
          ...
          for (int i=0; i < cnt_costs2.size(); ++i) cnt_costs2[i] *= sqp_config.merit_error_coeff_;
        ...
        double old_merit = vecSum(results_.cost_vals) + sqp_config.merit_error_coeff_ * vecSum(results_.cnt_viols);
        double model_merit = vecSum(model_cost_vals) + sqp_config.merit_error_coeff_ * vecSum(model_cnt_viols);
        double new_merit = vecSum(new_cost_vals) + sqp_config.merit_error_coeff_ * vecSum(new_cnt_viols);
