...
struct MultiCritFilter {
  ...
  OptStatus BasicTrustRegionSQP::optimize()
  {
    ...
    for (int merit_increases = 0; merit_increases < sqp_config.max_merit_coeff_increases_; ++merit_increases)
    ...
    penaltyadjustment:
      if (results_.cnt_viols.empty() || vecMax(results_.cnt_viols) < sqp_config.cnt_tolerance_)
      {
        if (results_.cnt_viols.size() > 0)
          LOG_DEBUG("woo-hoo! constraints are satisfied (to tolerance %.2e)", sqp_config.cnt_tolerance_);
        goto cleanup;
      }
      else
      {
