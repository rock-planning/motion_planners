...
inline Vector3d toVector3d(const geometry::Vector& v) {
  return Vector3d(v.x, v.y, v.z);
}
inline Vector4d toVector4d(const geometry::Vector& v) {
  return Vector4d(v.x, v.y, v.z, v.w);
}
Eigen::Matrix3d toRot(const geometry::Vector& rq);

inline geometry::Transform toRaveTransform(const Vector4d& q, const Vector3d& p) {
  return geometry::Transform(geometry::Vector(q[0], q[1], q[2], q[3]),
                       geometry::Vector(p[0], p[1], p[2]));
}
