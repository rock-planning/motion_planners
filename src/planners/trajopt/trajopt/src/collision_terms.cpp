...
namespace trajopt
{
                                       ...
                                       const VarVector &vars, const DblVec &dofvals, vector<AffExpr> &exprs, bool isTimestep1)
  {
    ...
    BOOST_FOREACH (const Collision &col, collisions)
    {
      if ((!col.linkA.empty() || col.linkA != "") && (!col.linkB.empty() || col.linkB != ""))
      {
        ...
        const Vector3d &pt = is_linkA ? col.ptA : (isTimestep1 && (col.cctype == CCType_Between)) ? col.ptB1
                                                                                                  : col.ptB;
  ...
  SingleTimestepCollisionEvaluator::SingleTimestepCollisionEvaluator(ConfigurationPtr rad, CollisionCheckerPtr coll, const VarVector &vars) : m_rad(rad),
                                                                                                                                              m_cc(coll),
                                                                                                                                              m_vars(vars),
                                                                                                                                              m_filterMask(-1)
  {
  ...
  CastCollisionEvaluator::CastCollisionEvaluator(ConfigurationPtr rad, CollisionCheckerPtr coll, const VarVector &vars0, const VarVector &vars1) : m_rad(rad),
                                                                                                                                                   m_cc(coll),
                                                                                                                                                   m_vars0(vars0),
                                                                                                                                                   m_vars1(vars1)
  {
  ...
  CollisionCost::CollisionCost(double dist_pen, double coeff, ConfigurationPtr rad, CollisionCheckerPtr coll, const VarVector &vars) : Cost("collision"),
                                                                                                                                       m_calc(new SingleTimestepCollisionEvaluator(rad, coll, vars)), m_dist_pen(dist_pen), m_coeff(coeff)
  {
  }

  CollisionCost::CollisionCost(double dist_pen, double coeff, ConfigurationPtr rad, CollisionCheckerPtr coll, const VarVector &vars0, const VarVector &vars1) : Cost("cast_collision"),
  ...
  CollisionConstraint::CollisionConstraint(double dist_pen, double coeff, ConfigurationPtr rad, CollisionCheckerPtr coll, const VarVector &vars) : m_calc(new SingleTimestepCollisionEvaluator(rad, coll, vars)), m_dist_pen(dist_pen), m_coeff(coeff)
  {
    name_ = "collision";
  }

  CollisionConstraint::CollisionConstraint(double dist_pen, double coeff, ConfigurationPtr rad, CollisionCheckerPtr coll, const VarVector &vars0, const VarVector &vars1) : m_calc(new CastCollisionEvaluator(rad, coll, vars0, vars1)), m_dist_pen(dist_pen), m_coeff(coeff)
  {
    name_ = "collision";
  }
