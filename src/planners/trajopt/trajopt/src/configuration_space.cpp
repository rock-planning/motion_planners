...
namespace trajopt
{
  ...
  void RobotAndDOF::GetDOFLimits(DblVec &lower, DblVec &upper) const
  {
    ...
    if (affinedofs & DOF_RotationMask)
    {
      ...
      else if (affinedofs & DOF_RotationQuat)
      {
        ...
        throw OR::openrave_exception("invalid rotation dofs", ORE_InvalidArguments);
    }
  }
  ...
  bool RobotAndDOF::DoesAffect(const KinBody::Link &link)
  {
    if (affinedofs > 0)
      return true;
    else if (link.GetParent() == GetRobot())
      return trajopt::DoesAffect(*GetRobot(), joint_inds, GetRobotLinkIndex(*GetRobot(), link));
    else
      return false;
  }
